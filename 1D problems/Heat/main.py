...
def Analytical_solution(x, t, m=100):
    λ = lambda n: (2*n - 1)*np.pi / 2
    A_n = lambda n: -12*((-1)**n + 1/λ(n)) / λ(n)**3
    
    F = 0
    for n in range(1, m+1):
        F += A_n(n) * np.exp(-λ(n)**2*t) * np.cos(λ(n)*x)
...
class HeatPinn(nn.Module):
    def __init__(self, model, k=1.0, w_f=1.0, w_bc=1.0, w_ic=1.0, device="cpu"):
        super().__init__()

        self.k = k # heat conductivity coefficient
        self.w_f = w_f
        self.w_bc = w_bc
        self.w_ic = w_ic
    ...
    def pde_loss(self):
        ...
        loss = torch.mean((u_t - self.k * u_xx)**2)
    ...
    def bc_loss(self):
        ...
        left_side = (self.x_bnd == 0.0)
        left_loss = torch.mean((u_pred_x[left_side])**2)
        right_loss = torch.mean((u_pred[~left_side])**2)

        # left_loss = self.mse(u_pred_x[left_side], self.u_bnd[left_side])
        # right_loss = self.mse(u_pred[~left_side], self.u_bnd[~left_side])   
    ...
    def train(self, X_f, X_bnd, u_bnd, X_ic, u_ic, epochs=1000):
        ...
        print(f"[L-BFGS] Optimization finished, final values\n"
...
if __name__ == "__main__":
    ...
    N_b = 100 # boundary points for each side
    N_i = 200 # initial points

    t_b = np.random.uniform(low=t_bounds[0], high=t_bounds[1], size=(N_b, 1))
    left_side = np.hstack([np.zeros_like(t_b), t_b])
    right_side = np.hstack([np.ones_like(t_b), t_b])
    ...
    model = NeuralNet(input_dim=2, output_dim=1, hidden_layers=4, hidden_dim=80)
    pinn = HeatPinn(model, w_bc=100.0, w_ic=100.0, device=device)
    ...
    u_true = Analytical_solution(X, T, m=100)
